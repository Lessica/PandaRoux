...
public interface GroupeRepository extends CrudRepository<Groupe, Integer> {
    ...
    @Query("SELECT g.id AS id, g.name AS name, (SELECT COUNT(*) FROM g.lectures) AS lecture_count, (SELECT COUNT(*) FROM g.quizzes q WHERE q.active = true) AS quiz_count FROM Groupe g JOIN g.students s WHERE s.id = ?1")
